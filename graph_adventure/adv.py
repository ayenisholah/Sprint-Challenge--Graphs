...
def breadth_first_traversal(player, room_queue):
    current_room = rooms[player.currentRoom.id]
    unvisted_room = []
    for direction in current_room:
        if current_room[direction] == None:
            unvisted_room.append(direction)
    if len(unvisted_room) > 0:
        room_queue.enqueue(unvisted_room[0])
    else:
        unexplored_path = traverse_unvisted_rooms(player, room_queue)

        current_path = player.currentRoom.id

        for path in unexplored_path:
            print(path)
            for direction in rooms[current_path]:
                if rooms[current_path][direction] == path:
                    room_queue.enqueue(direction)
                    current_path = path
                    break


def traverse_unvisted_rooms(player, room_queue):
    q = Queue()
    visited = set()
...
breadth_first_traversal(player, room_queue)
...
while room_queue.size() > 0:
    ...
    if end_room not in rooms:
        ...
        for direction in player.currentRoom.getExits():
            rooms[end_room][direction] = None

    rooms[end_room][change_direction(next_move)] = start_room
